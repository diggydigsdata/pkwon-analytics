...
CURRENT_SEASON = "2025-2026"  # excluded from historical export
...
def dedupe_rows(df: pd.DataFrame) -> pd.DataFrame:
    ...
    df = df.drop_duplicates(subset=["player","team","comp","season"], keep="first")
...
def main():
    ...
    canon_str = ["player","team","comp","season","nationality","country_code","position"]
    ...
    historical.to_csv(OUTPUT_DIR / "big5_ucl_pkwon_final.csv", index=False, encoding="utf-8")
    try:
        historical.to_parquet(OUTPUT_DIR / "big5_ucl_pkwon_final.parquet", index=False)
    except Exception as e:
